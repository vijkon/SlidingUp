    ...
    : ActivityCompat.OnRequestPermissionsResultCallback
{
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray)
    {
        ...
        if(!isPermissionGranted)
        {
            ...
            if(should)
                additionalInfoDialog(ACCESS_FINE_LOCATION)
            else
    ...
    private fun additionalInfoDialog(permission : String)
    {
        ...
        if (should) {
            ...
            builder.setNegativeButton(R.string.re_try) { dialog, _ ->
                dialog.dismiss()
                ActivityCompat.requestPermissions(activity, arrayOf(permission),PERMISSIONS_REQUEST_LOCATION)
            }
            builder.show()
        }
    }
